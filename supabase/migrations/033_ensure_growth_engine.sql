-- ============================================================
-- Migration: 033_ensure_growth_engine
-- Goal: Idempotent re-deployment of the full growth engine.
--       Safe to run even if 026 was already applied.
--       Fixes: 0 streak / 0 score / missing awarded_points / all_tasks_complete.
-- ============================================================

-- 1. Ensure user_tree table + columns exist
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'user_tree' AND policyname = 'tree_own'
  ) THEN
    CREATE POLICY "tree_own" ON public.user_tree FOR ALL USING (auth.uid() = user_id);
  END IF;
END $$;

-- 2. Ensure tree_daily_log table + columns exist
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'tree_daily_log' AND policyname = 'log_own'
  ) THEN
    CREATE POLICY "log_own" ON public.tree_daily_log FOR ALL USING (auth.uid() = user_id);
  END IF;
END $$;

-- 3. Ensure user_behavior table + columns exist
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'user_behavior' AND policyname = 'behavior_own'
  ) THEN
    CREATE POLICY "behavior_own" ON public.user_behavior FOR ALL USING (auth.uid() = user_id);
  END IF;
END $$;

-- 4. Ensure loop_tasks has required columns
...
-- Sync done ↔ completed_at for existing rows
UPDATE public.loop_tasks
SET completed_at = COALESCE(completed_at, created_at, now())
WHERE COALESCE(done, false) = true AND completed_at IS NULL;

UPDATE public.loop_tasks
SET done = true
WHERE completed_at IS NOT NULL AND COALESCE(done, false) = false;
...
-- 6. calc_vitality helper function
...
-- 7. Full complete_loop_task_v4 — returns all fields the frontend expects:
--    new_vitality, new_total_completed_today, today_tasks_total,
--    new_streak, new_score, awarded_points, all_tasks_complete
...
BEGIN
  ...
  -- Count completed core categories today
  ...
  -- Lifetime total for user_behavior
  SELECT COUNT(*)
