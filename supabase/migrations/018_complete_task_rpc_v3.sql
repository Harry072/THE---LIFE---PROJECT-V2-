-- Migration: 018_complete_task_rpc_v3
-- Atomic task completion and metrics update targeting the `tasks` table

CREATE OR REPLACE FUNCTION complete_daily_task(p_task_id UUID)
RETURNS JSON AS $$
DECLARE
  v_user_id UUID;
  v_domain task_domain;
  v_is_done BOOLEAN;
  v_points INTEGER;
...
BEGIN
  ...
  -- 2. Handle Already Done state gracefully
  IF v_is_done THEN
  ...
  -- 3. Update task (using completed_at for tasks table instead of done boolean)
  UPDATE tasks
  SET completed_at = now()
  WHERE id = p_task_id;

  -- 4. Calculate points (Base logic = 10 points)
  v_points := 10;

  -- 5. Update tree_daily_log
  INSERT INTO tree_daily_log (user_id, for_date, points, tasks_done, tasks_total)
  VALUES (v_user_id, v_assigned_date, v_points, 1, 
         (SELECT count(*) FROM tasks WHERE user_id = v_user_id AND assigned_date = v_assigned_date))
